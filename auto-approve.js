...
async function dispatchOrderToRider(orderId) {
  ...
  // Each transition is a separate transaction because Firestore rules validate
  // pending → accepted → preparing → ready as individual merchant actions.
...
// app.js handles every order action with one document-level bubble listener.
// Capture this specific action first so one merchant tap creates the rider-ready
// state without the old three-tap gap that prevented delivery requests.
